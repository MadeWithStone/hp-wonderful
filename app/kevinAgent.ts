...
class KevinAgent {
    ...
    static amplitudeCallbacks: ((amplitude: number) => void)[] = [];

    static addAmplitudeCallback = (callback: (amplitude: number) => void) => {
        this.amplitudeCallbacks.push(callback);
    }
    ...
    static async speak(text: string) {
        try {
            ...
            sound.setOnAudioSampleReceived((sample) => {
                ...
                const normalizedAmplitude = Math.min(1, rms / 32768); // Assuming 16-bit PCM
                // console.log("Normalized Amplitude:", normalizedAmplitude);
                this.amplitudeCallbacks.forEach((callback) => callback(rms));
